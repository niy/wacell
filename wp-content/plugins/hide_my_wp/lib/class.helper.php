...
if (!class_exists('PP_Helper')) {
    ...
    class PP_Helper {
        ...
        function register_messages(){
            ...
            if( !wp_next_scheduled( 'pp_important_messages2' ) )   
                wp_schedule_event( time(), 'once_3days', 'pp_important_messages2' ); //or daily 
            else
                add_action( 'pp_important_messages2', array(&$this, 'update_pp_important_messages') );
        ...
        function update_pp_important_messages() {  
            ...
            if (is_multisite())
               $opt = get_blog_option(SITE_ID_CURRENT_SITE, self::slug);
            else
               $opt = get_option($this->slug) ;
               
            $url = 'ht'.'tp'.':/'.'/'.'wpwave.com/important_message.php?site='.urlencode(str_replace('ht'.'tp'.':/'.'/','',home_url())).'&wp_ver='.$wp_version.'&plugin='.$this->slug.'&ver='.$this->ver.'&li='.$opt['li'];
        ...
        function admin_notices() {
        	...
        	if (is_super_admin() && isset($recent_message['content']) && (!$dismiss_mesaages || !in_array($recent_message['id'], $dismiss_mesaages)) ){               
          	    if ( !$this->ends_with($_SERVER["PHP_SELF"],'options.php') && !$this->ends_with($_SERVER["PHP_SELF"],'network/index.php') && isset($recent_message['di']) && $recent_message['di'])
