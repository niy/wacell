...
if(!class_exists('avia_breadcrumb'))
{
	...
	function avia_breadcrumb($options = ""){
		...
		if(is_404()){ return __("404 - صفحه‌ای یافت نشد",'avia_framework'); }
		
		if(is_search()){ return __("جستجو",'avia_framework');}	
...
function avia_breadcrumbs( $args = array() ) {
	...
	$defaults = array(
		'separator' => '&raquo;',
		'before' => '<span class="breadcrumb-title">' . __( '', 'avia_framework' ) . '</span>',//شما اینجا هستید
		'after' => false,
		'front_page' => true,
		'show_home' => __( 'صفحه اصلی', 'avia_framework' ),
		'echo' => false, 
	...
	elseif ( is_search() )
		$trail['trail_end'] = 'نتیجه جستجو برای "' . esc_attr( get_search_query() ).'"';

	/* If viewing a 404 error page. */
	elseif ( is_404() )
		$trail['trail_end'] = __( '404 صفحه‌ای یافت نشد', 'avia_framework' );
	...
	if ( is_array( $trail ) ) {
		...
		if ( !empty( $trail['trail_end'] ) )
			$trail['trail_end'] = '<'.$el_tag.' class="trail-end">' . avia_backend_truncate($trail['trail_end'], $truncate, " ", $pad="...")  . '</'.$el_tag.'>';
