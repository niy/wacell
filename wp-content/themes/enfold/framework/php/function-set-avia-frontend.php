...
if(!function_exists('avia_which_archive'))
{
	...
	function avia_which_archive()
	{
		...
		if ( is_category() )
		{
			$output = __('بایگانی برای دسته بندی:','avia_framework')." ".single_cat_title('',false);
		}
		elseif (is_day())
		{
			$output = __('بایگانی برای تاریخ:','avia_framework')." ".get_the_time('F jS, Y');
		}
		elseif (is_month())
		{
			$output = __('بایگانی برای ماه:','avia_framework')." ".get_the_time('F, Y');
		}
		elseif (is_year())
		{
			$output = __('بایگانی برای سال:','avia_framework')." ".get_the_time('Y');
		}
		elseif (is_search())
		{
			global $wp_query;
			if(!empty($wp_query->found_posts))
			{
				...
				{
					$output =  $wp_query->found_posts ." ". __('نتیجه‌ی جستجو برای:','avia_framework')." ".esc_attr( get_search_query() );
				}
				else
				{
					$output =  $wp_query->found_posts ." ". __('نتیجه‌ی جستجو برای:','avia_framework')." ".esc_attr( get_search_query() );
			...
			else
			{
				if(!empty($_GET['s']))
				{
					$output = __('نتیجه‌ی جستجو برای:','avia_framework')." ".esc_attr( get_search_query() );
				}
				else
				{
					$output = __('برای جستجوی این سایت یک کلمه صحیح وارد نمایید','avia_framework');
				}
			}
		...
		elseif (is_author())
		{
			...
			$output = __('بایگانی نویسنده','avia_framework')." ";
		...
		elseif (is_tag())
		{
			$output = __('بایگانی برچسب برای:','avia_framework')." ".single_tag_title('',false);
		}
		elseif(is_tax())
		{
		...
		{
			$output = __('بایگانی','avia_framework')." ";
		}
