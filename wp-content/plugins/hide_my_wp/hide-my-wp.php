...
class HideMyWP {
    ...
    private $s;
    private $sub_folder; //subfolder installation type
    ...
    function admin_notices(){
        ...
        if (isset($_GET['page']) && $_GET['page']==self::slug && isset($_GET['settings-updated']) && is_multisite() )
        ...
        if (isset($_GET['page']) && $_GET['page']==self::slug && stristr($_SERVER['SERVER_SOFTWARE'], 'iis') || stristr($_SERVER['SERVER_SOFTWARE'], 'Windows') )
            echo '<div class="error"><p>' . __('It seems you use Windows web server! Hide My WP doesn\'t offically support IIS but you may still make it work! Read the <a href="http://codecanyon.net/item/hide-my-wp-no-one-can-know-you-use-wordpress/4177158/faqs/16072">FAQ</a>', self::slug ) . '</p></div>';
   ...
   /**
    ...
    function wp(){
        ...
        if ((is_tax() || is_post_type_archive() || is_trackback() || is_comments_popup() || is_attachment()) && !isset($_GET['post_type']) && !isset($_GET['taxonamy']) && !isset($_GET['attachment']) && !isset($_GET['attachment_id']) && $this->opt('disable_other_wp'))
            $this->block_access();
        if (!is_404() && (stristr($_SERVER['HTTP_USER_AGENT'], 'BuiltWith') || stristr($_SERVER['HTTP_USER_AGENT'], '2ip.ru')) )
            $this->block_access();
        ...
        if ($this->opt('remove_other_meta')){
         ...
         if ( $this->opt('new_style_name') && $this->opt('new_style_name')!='style.css' && $this->is_permalink() && !isset($_POST['wp_customize']) )  {
            ...
            if (is_multisite()){
                ...
                $rel_style_path = $blog_path . $rel_theme_path_with_theme .'/style.css'; //without theme
...
;
                ...
                if (is_multisite()){
                    ...
                    $rel_this_plugin_path = $blog_path .str_replace($this->sub_folder,'',$rel_this_plugin_path);  
                }
        ...
        if ($this->opt('new_include_path') && $this->is_permalink()){ 
            ...
            if (is_multisite()){
                ...
                $rel_include_path = $blog_path .str_replace($this->sub_folder,'',$rel_include_path);  
            }
        ...
        if ($this->opt('new_upload_path') && $this->is_permalink()){
            ...
            if (is_multisite()){
                ...
                $rel_upload_path = $blog_path .str_replace($this->sub_folder,'',$rel_upload_path);  
            }
            ...
            if ($this->opt('replace_javascript_path')> 2) {
                $this->replace_old[]= str_replace('/', '\/', site_url($rel_upload_path));
                $this->replace_new[]= str_replace('/', '\/', site_url($new_upload_path));   
            }
        }
        ...
        if ($this->opt('new_plugin_path') && $this->is_permalink()){    
            ...
            if (is_multisite()){
                ...
                $rel_plugin_path = $blog_path .str_replace($this->sub_folder,'', $rel_plugin_path);  
            }
        ...
        if ($this->opt('new_theme_path') && $this->is_permalink() && !isset($_POST['wp_customize'])){
            ...
            if (is_multisite()){   
                ...
                $rel_theme_path_with_theme = trim(str_replace(site_url(),'', get_stylesheet_directory_uri()), '/');
                $rel_theme_path = $blog_path . str_replace('/'.get_stylesheet(), '', $rel_theme_path_with_theme); //without theme  
            }
        ...
        if ($this->opt('replace_admin_ajax') && trim($this->opt('replace_admin_ajax'), '/ ')!='admin-ajax.php' && trim($this->opt('replace_admin_ajax') )!='wp-admin/admin-ajax.php' && $this->is_permalink())  {
            ...
            $new_admin_ajax = trim($this->options['replace_admin_ajax'], '/ ');
            ...
            if (is_multisite()){
                ...
                $rel_admin_ajax = $blog_path .str_replace($this->sub_folder,'',$rel_admin_ajax);  
            }
        ...
        if ($this->opt('replace_comments_post') && trim($this->opt('replace_comments_post'), '/ ')!='wp-comments-post.php' && $this->is_permalink())        {
            ...
            if (is_multisite()){
                ...
                $rel_comments_post = $blog_path .str_replace($this->sub_folder,'', $rel_comments_post);  
            }
...
;
        ...
        <ol>
            ...
            <li> You may need to re-configure the server whenever you change settings or activate a new theme or plugin. </li>
            <li>Add these lines before: RewriteCond %{REQUEST_FILENAME} !-f). Next you may want to change htaccess permission to read-only (e.g. 666)</li>
...
;
        ...
        if ($new_style_name)
            $output.='RewriteRule ^'.$new_theme_path. '/([_0-9a-zA-Z-]+)/'.$new_style_name.' /index.php?style_wrapper=true&template_wrapper=$1 [QSA,L]'."\n";
        ...
        if (isset($_GET['multisite_config']) && $_GET['multisite_config'])  {
            ...
            $html.= sprintf( '<span class="description">
            <ol>
            <li>Add below lines after wp-admin or files section but before RewriteCond %{REQUEST_FILENAME} !-f </li>
