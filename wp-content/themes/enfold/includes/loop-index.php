...
if (have_posts()) :
	...
	$current_post['content'] 	= $blog_content == "content" ? get_the_content(__('ادامه مطلب','avia_framework').'<span class="more-link-arrow">  &rarr;</span>') : get_the_excerpt();
	$current_post['content'] 	= $blog_content == "excerpt_read_more" ? $current_post['content'].'<div class="read-more-link"><a href="'.get_permalink().'" class="more-link">'.__('ادامه مطلب','avia_framework').'<span class="more-link-arrow">  &rarr;</span></a></div>' : $current_post['content'];
	...
	echo "<div class='".implode(" ", get_post_class('post-entry post-entry-type-'.$post_format . " " . $post_class . " ".$with_slider))."'>";
			...
			echo "<div class='entry-content clearfix {$post_format}-content'>";
				...
				if(has_tag() && is_single())
				{
					...
					the_tags('<strong>'.__('برچسب‌ها:','avia_framework').'</strong><span> ');
					echo '</span></span>';
				}
...
<div class="entry">
<h2 class='post-title'><?php _e('چیزی یافت نشد', 'avia_framework'); ?></h2>
